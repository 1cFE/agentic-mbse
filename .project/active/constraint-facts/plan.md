...
- [x] **`src/agentic_mbse/sysml/constraint_extraction.py`** (NEW) — `extract_constraint_facts(model) -> ConstraintFacts` + private classify/recover helpers. Only module touching syside.
  - [x] **Sweep** `elements_of_type(ConstraintUsage, include_subtypes=True)` (`syside_adapter.py:270`). NOT an `AssertConstraintUsage`-rooted sweep (misses `satisfy` — `[HARD]`).
  - [x] **Classify (MF2 — restore S1's gate order, `constraint_fact_learning.py:164-177`):** membership gate (`RequirementConstraintMembership` → `requirement_constraint`, kind from `.kind`) → assert gate (`isinstance(AssertConstraintUsage)`; inside: `asserted_constraint is not self` → `named_usage_reference`; else no owned `result_expression` → `definition_typed`; else → `inline`) → satisfy gate (`isinstance(SatisfyRequirementUsage)` → `satisfy`) → fallback `plain_usage`. The `result_expression`-ownership test is the **within-assert** `[HARD]` replacement for the banned namespace-prefix test — **not** a whole-population classifier.
  - [x] **Membership kind** from the owning `RequirementConstraintMembership.kind`, neutralized (`.name.lower()`), never the usage subtype (`[HARD]`).
  - [x] **Formals** by owner-filtered `AttributeUsage` sweep (`owner is definition`); default = owned `FeatureValue` with `is_default=true`. NOT `ConstraintDefinition.parameters` (`[HARD]`).
  - [x] **`owning_definition` (D6/MF3, tagged + total):** walk `owner` up to the first enclosing `PartDefinition`/`CalculationDefinition`/`RequirementDefinition`/`Package`; a `Package` always terminates → `kind=package` (e.g. `direct_owned`). Never falls through.
  - [x] **Operand leaf facts:** category by type conformance; enumeration by owning `EnumerationDefinition`; **dimension via the structural path (MF4)** — unit-annotation operand: the `[` operator's unit operand `referent` → its typing chain's unit-def QN (select the type specializing `MeasurementUnit`/`SimpleUnit`, not positional `[0]`); quantity-feature operand: follow the value type's `mRef` to its most-specific unit-definition type (the addendum's confirmed traversal), `unit=null`, `dimension`=that QN. **No `removesuffix("Unit")`/`removesuffix("Value")`.**
  - [x] **Neutralize library values:** `FeatureDirectionKind` → `in`/`out`/`inout`; enum-likes via `.name.lower()`. NOT `str(enum)` (`[HARD]` M3).
  - [x] **Non-finite literal → structured extraction diagnostic** naming the operand + source location (D2a); not a serialize crash.
- [x] Export `extract_constraint_facts` from `src/agentic_mbse/sysml/__init__.py`.

### Validation
**Automated:**
- [x] `uv run pytest tests/test_sysml/test_constraint_extraction.py` → all pass (14/14); six forms distinct; `requirement_constraint` not misclassified; dimension is `ISQBase::LengthUnit`; direction is a neutral token; non-finite yields a diagnostic.
- [x] `uv run pytest tests/` → no regressions (1314 passed, up from 1300).
- [x] `uv run ruff check src/ tests/ && uv run mypy src/` → clean on all touched files (pre-existing unrelated errors elsewhere, unchanged).
...
**Completed:** 2026-07-12
**Actual Changes:**
- Created `src/agentic_mbse/sysml/constraint_extraction.py`: `extract_constraint_facts(model)` + private classify/recover helpers (`_classify`, `_effective_predicate_source`, `_source_fact`, `_owning_definition`, `_membership_kind`, `_definition_formals`, `_actuals`, `_omitted_default_formals`, `_inherited_into`, `_context_fact`, `_expression_fact` + leaf builders, `_operand_type_fact`, `_unit_annotation_fact`, `_unit_definition_qn`, `_quantity_feature_dimension`).
- Added `Package`, `RequirementConstraintMembership`, `EnumerationDefinition`, `FeatureValue` to `syside_adapter.py`'s `TYPE_MAP` (purely additive — no existing entry's behavior changed).
- Exported `extract_constraint_facts` from `src/agentic_mbse/sysml/__init__.py`.
- Added `tests/test_sysml/test_constraint_extraction.py` (14 tests, live SysIDE over both S1 fixtures) — all pass, including the full 14-case `type_units.sysml` equality matrix spot-checked manually against the S1 golden's category/enumeration/unit values (dimension asserted as `ISQBase::LengthUnit`/`MassUnit`, not the golden's strip artifact).

**Issues:**
- None — B1 held on the first live run against both fixtures; no discriminator divergence.

**Deviations:**
- **`ConstraintSource` field population is unconditional, not form-gated**, for three of its four pointer fields. Design's schema sketch implies (and my Phase 1 docstring wrongly stated) that only one pointer field is set per form. The S1 golden shows otherwise: `constraint_definition` and `asserted_constraint` are **raw, unconditional** reads of `constraint.constraint_definition`/`constraint.asserted_constraint` (populated for every form that has them, including `requirement_constraint` and `satisfy` — e.g. a `satisfy` usage's raw `constraint_definition` resolves to the `RequirementDefinition` it satisfies). Only `effective_predicate_source` is form-derived (S1's `_effective_predicate_source` dispatch). Implemented to match the golden exactly; corrected the Phase 1 docstring in `constraint_facts.py` to describe this accurately instead of the "exactly one" claim.
- **`operand_type` is computed for every non-Boolean-connective `ExpressionFact` node, not only `FeatureReferenceExpression`/literal leaves as design's Core Concept literally states ("operand_type hangs off every leaf-bearing node... non-leaf operator nodes carry operand_type=None").** The `type_units.sysml` fixture's `unit_bearing_arithmetic` case (`1 [m] + 1 [m] == 2 [m]`) requires a compound `+` node to carry a resolved `quantity`/`SI::metre` fact to match the S1 golden — a `+`/`-` node is not Boolean, it's a value. Similarly the unit-annotation `[` operator node (`1 [m]` itself) is a leaf-bearing operator, not a `FeatureReferenceExpression`/literal. Resolved as: `operand_type=None` only for the fixed set of Boolean-relational/connective operators (`==`,`!=`,`<`,`>`,`<=`,`>=`,`and`,`or`,`not`,`implies`); every other node (references, literals, `[`, arithmetic, anything else) gets `operand_type` computed via `cached_result_type` + structural unit resolution. Verified against all 14 `type_units.sysml` equality cases — every category/enumeration/unit value matches the S1 golden semantics exactly (dimension updated per MF4). This is a technical implementation-level resolution of an ambiguity between two design statements, not a scope or goal-level conflict, so I resolved it rather than stopping the run — flagging it here per the surfacing law for review.
- **`ConstraintFacts.definitions` and `.contexts` are NOT namespace-filtered** (S1's capture module filtered both to the `ConstraintFactShapeProbe::` probe package — a fixture-coupled heuristic that has no place in production). `definitions` = the deduplicated set of `ConstraintDefinition` elements actually referenced as some usage's `constraint_definition` (not a blanket model-wide sweep, which would include unrelated library constraint defs). `contexts` = every `PartDefinition`/`PartUsage`/`CalculationDefinition` in the model that has at least one non-empty `general_types`/`inherited_constraints`/`redefinitions` fact — a structural relevance filter, not a namespace one. Verified this reproduces exactly the S1 golden's `definitions`/`contexts` membership on the fixtures (1 definition, 2 contexts) despite the different filtering principle.
- **Extraction diagnostics carry only D2a's non-finite-literal case**, not S1's loader/parse diagnostics (`capture_all_facts`'s `source_forms.diagnostics`/`type_units.diagnostics`). Model-load diagnostics are the caller's concern (they come back from `syside.try_load_model` independently of `extract_constraint_facts`); folding them into `ConstraintFacts.diagnostics` would conflate two different diagnostic sources under one field with no spec requirement to do so.
